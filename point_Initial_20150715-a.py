...
# coupue des entités vectorielles selon la limite de la zone tampon
...
#os.remove(dossierRaster)
#os.removedirs(dossierRaster)
#os.makedirs(dossierRaster)


##for filename in os.listdir(dossierRaster) :
##    os.remove(dossierRaster + "/" + filename)


zoneTampon ="Zone_tampon_p"


rows = arcpy.SearchCursor("CLIP_TABLE","","","entree; rasterTIF; TypeCou","TypeCou A")      #  coordinate

#  rows = arcpy.SearchCursor("CLIP_TABLE","",coordinate,"entree; rasterTIF; TypeCou","TypeCou A")      #  coordinate
...
try:
    ...
    for row in rows:
        couche = row.getValue("entree")
        nomRaster = row.getValue("rasterTIF")
        arcpy.AddMessage(nomRaster)
        arcpy.Clip_management(couche,"",nomRaster,zoneTampon,"0","ClippingGeometry")  # original
        #extraction = arcpy.sa.ExtractByMask(couche,zoneTampon)  # nouvea
        #extraction.save(nomRaster) # nouveau


        # arcpy.Clip_management("mnt","","mnt_coupe","Zone_tampon_p","0","")  #  ClippingGeometry
       #  arcpy.Clip_management()
        # arcpy.Clip_analysis(couche,zoneTampon,gdb)
...
C_circa = r"D:\MAITRISE\isochrones\Donnees\CIRCA_2000.gdb\CIRCA2000"


arcpy.Clip_analysis(C_circa,zoneTampon,"circa_clip")
arcpy.AddMessage(C_circa)


# ################################
# Add Layer Clip_CIRCA
# ###############################
...
addLayer = arcpy.mapping.Layer(r"D:\MAITRISE\isochrones\Recherche\LAYERS\circa_clip.lyr")
arcpy.mapping.AddLayer(df, addLayer, "AUTO_ARRANGE")
arcpy.FeatureToRaster_conversion("circa_clip","code_circa.Fact","matric_circa", "21")  #  \matric_circa  changer le bacslash  # tester  21.1595672
# arcpy.FeatureToRaster_conversion("circa_clip","code_circa.Fact",couchesRasters+"/matric_circa", "15")  #  \matric_circa  changer le bacslash



# ################################
# Mosaïque de tous couvertures rasters dans la couche matric_circa.
# Correspond à la carte de cout de superficie
# ###############################

##    arcpy.env.workspace = couchesRasters

# arcpy.Mosaic_management("route;Sentier; Chem_fer; Li_tr_en; Cou_eau; Reg_hyd ","matric_circa","LAST","FIRST",
#                        "", "", "", "", "")

# assigner la valeur 0.01 au raster Reg_hyd pour créer des barrieres aux isocrhones

try:

    hyd = Raster("Reg_hyd")/100.00    # 100.00 avec Power

    hyd.save("hyddro")

except:
    pass
    arcpy.AddMessage(u"Impossible de recalculer Reg_hyd")  # voir si ajoute messages pour les mosaiques




#  mosaiques des rasters

try:
    arcpy.Mosaic_management("Li_tr_en","hyddro","FIRST","MATCH","", "", "", "", "")  # FIRST

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Li_tr_en")  # voir si ajoute messages pour les mosaiques


try:

    arcpy.Mosaic_management("hyddro","matric_circa","LAST","MATCH","", "", "", "", "") # Reg_hy

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Hydro")  # voir si ajoute messages pour les mosaiques

#  mosaiques des rasters

try:

    arcpy.Mosaic_management("Cou_eau","matric_circa","LAST","MATCH","", "", "", "", "")

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Cou_eau")  # voir si ajoute messages pour les mosaiques
...
try:
    ...
    arcpy.Mosaic_management("Lig_tra","matric_circa","LAST","MATCH","", "", "", "", "")


except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Lig_tra")  # voir si ajoute messages pour les mosaiques



try:
    arcpy.Mosaic_management("route","matric_circa","LAST","MATCH","", "", "", "", "")  # FIRST

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter route")  # voir si ajoute messages pour les mosaiques


try:
    arcpy.Mosaic_management("Chem_fer","matric_circa","LAST","MATCH","", "", "", "", "")
except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter chem_fer")  # voir si ajoute messages pour les mosaiques



try:

    arcpy.Mosaic_management("Senti","matric_circa","LAST","MATCH","", "", "", "", "")

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Senti")  # voir si ajoute messages pour les mosaiques



try:

    arcpy.Mosaic_management("Bati","matric_circa","LAST","MATCH","", "", "", "", "")  # voir s il y a d erreur

except:
    pass

    arcpy.AddMessage(u"Impossible d'ajouter Batime")  # voir si ajoute messages pour les mosaiques
...
# ################################
# clip modèle numérique du terrain (mnt)
# ###############################


arcpy.Clip_management("mnt","","mnt_coupe","Zone_tampon_p","0","")  #  ClippingGeometry

# arcpy.Clip_management("mnt_coupe","mnt_coup_zt","","Zone tampon","0","ClippingGeometry")  #
...
outSlope = Slope("mnt_coupe", "DEGREE", 1)

outSlope.save("CartePente")
...
##elif float(imc)> 18.5 and float(imc) <= 24.9:
##    Fp = 1.0
...
# varvitese = (Fvf *arcpy.sa.Exp(-3.5* (arcpy.sa.Abs((arcpy.sa.Tan((((arcpy.sa.Raster("CartePente"))/57.29578))+0.05)))))) # C etat mon calcul original
# varvitese = (Fvf *arcpy.sa.Exp(-3.5* (arcpy.sa.Abs((arcpy.sa.Tan((((arcpy.sa.Raster("CartePente"))/(180.0/nPi)))+0.05))))))  # tester cette formule plutôt
varvitese = (Fvf *Exp(-3.5* (Abs((Tan((((Raster("CartePente"))*(nPi/180.0)))+0.05))))))  # tester cette formule plutôt
# varvitese = (Fvf*arcpy.sa.Exp(-3.5*(arcpy.sa.Abs((arcpy.sa.Raster("CartePente")/100.00)+0.05))))  # cette formule était jusque 2015-04-01

 # voir aussi Fvipa*

##varsitese = (6*arcpy.sa.Exp(-3.5* (arcpy.sa.Abs(arcpy.sa.Raster("CartePente"))/57.29578)))

# vitmetsec = varvitese/3600.00   #  vitesse en mts/sec   ORIGINAL

vitmetsec = (varvitese*1000.00)/3600.00   #  vitesse en mts/sec

# vitmetsec = 3600.00/(varvitese*1000.00)  #  vitesse en sec/mts.  |Don
# varvitese.save("CarteViteseSup")  original
...
##Cv = (arcpy.sa.Raster("CarteViteseSup") / (arcpy.sa.Raster("matric_circa")/100.000)) #  attention  avant j'avais utilisé multiplication
##
##Cv.save("CartVitVoyage")  #  Carte de vitese de voyage en km/h
...
Cvmult = (Raster("CarteViteseSup") * (Raster("matric_circa")/100.000)) #
...
##maxValeur = arcpy.GetRasterProperties_management(Cvmult,"MAXIMUM")
##maxValSort = maxValeur.getOutput(0)
##arcpy.AddMessage(float(maxValSort))
##
##minvaleur = arcpy.GetRasterProperties_management(Cvmult, "MINIMUM")
##
##minValSort = minvaleur.getOutput(0)
##arcpy.AddMessage(float(minValSort))
##
##inverse = (((arcpy.sa.Float(Cvmult) - float(maxValSort)) * (-1)) + float(minValSort)) + 1.00  # ajout de plus 1 voir qui passe
##
##inverse.save("CrtVitVoyMul")
...
##
##IsoC1hr1 = arcpy.sa.CostDistance("PI", arcpy.sa.Raster("CrtVitVoyMul"), "","isochor1" )  # Valeur en secondes multiplication
##
##
##IsoC1hr1.save("isochor1")
##
##IsoC1hr2 =arcpy.sa.Float("isochor1")
##
##IsoC1hr3 = (IsoC1hr2 / 3600.00)  #  Valeur en heures
##
##IsoC1hr3.save("Isochrones")
...
PathDist = PathDistance("PI", Raster("CrtVitVoyMul"), Raster("mnt_coupe"), "", "", Raster("mnt_coupe"),vFACTOR,"","")  #
...
del actuDist, couche, couchesRasters, chaActivitePhysique, chaAge
...
del  nomRaster, rang, Point_initial, inactPhysique, inage, inHauteurneige
del  inheureDisparition, inpoids, inprecipitation, insexe, intemperature
del intaille, invents, rows, Comport, Fichier, addLayer, varvitese
del Fa
    ##curs
